...
def natural_param_continuation(initial_u, param_to_vary, param_range, no_param_values, function, discretisation=lambda x:x,
                                    solver='fsolve', phase_condition = None, **params):
    '''
    ...
    if discretisation == numerical_shooting:
        ...
        for i in range(len(param_list)):
            ...
            prev_sol = sols[i]
            X0, T = numerical_shooting(prev_sol.copy(), 5, function, phase_condition, **params)
            sols.append(list(X0))
...
def main():
    ...
    u, p = natural_param_continuation([1,1], 'beta', [2,-1], 20, hopf, numerical_shooting, phase_condition=pc_hopf, beta=-1, sigma=-1)
    plt.plot(p,u[:,0])
    plt.show()
